...
export default function CamerasPage() {
  ...
  return <section dir={language === 'ar' ? 'rtl' : 'ltr'} className="space-y-6 px-4 pb-8 sm:px-6 lg:px-0">
    <header className="flex flex-col gap-4 rounded-[28px] border border-white/10 bg-white/5 p-5 backdrop-blur-xl sm:flex-row sm:items-end sm:justify-between sm:p-7">
      ...
      <div className="flex flex-wrap gap-2"><select aria-label={c.location} value={locationId} onChange={(event) => setLocationId(event.target.value)} className="min-h-11 rounded-xl border border-white/10 bg-slate-950 px-3 text-sm"><option value="">{c.allLocations}</option>{locations.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>{canManageNvrs && <button onClick={() => setForm({ ...emptyForm, locationId: locationId || locations[0]?.id || '' })} className="flex min-h-11 items-center gap-2 rounded-xl bg-cyan-400 px-4 font-semibold text-slate-950"><Plus className="h-4 w-4" />{c.addNvr}</button>}<button onClick={() => void load()} aria-label={c.refresh} className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10"><RefreshCw className="h-4 w-4" /></button></div>
    </header>
    {error && <div role="alert" className="flex items-center justify-between rounded-2xl border border-rose-400/30 bg-rose-500/10 p-4 text-rose-100"><span>{error}</span><button onClick={() => void load()} className="min-h-11 px-3 font-semibold">{c.retry}</button></div>}
    ...
    {loading ? <div className="rounded-3xl border border-white/10 p-8 text-center text-slate-300">{c.loading}</div> : <>
      <div><h2 className="mb-3 text-xl font-bold">{c.nvrs}</h2>{visibleNvrs.length === 0 ? <div className="rounded-3xl border border-dashed border-white/15 p-8 text-center text-slate-400"><Server className="mx-auto mb-3 h-8 w-8" />{locations.length === 0 ? c.noLocations : c.noNvrs}</div> : <div className="grid gap-3 lg:grid-cols-2">{visibleNvrs.map((nvr) => <article key={nvr.id} className="rounded-2xl border border-white/10 bg-slate-950/70 p-5"><div className="flex justify-between gap-3"><div><h3 className="font-bold">{nvr.name}</h3><p className="text-sm text-slate-400">{nvr.vendor} · {locationName(nvr.location_id)}</p></div><span className="h-fit rounded-full bg-slate-800 px-3 py-1 text-xs">{c.status[nvr.status as keyof typeof c.status] ?? nvr.status}</span></div><dl className="mt-4 grid grid-cols-2 gap-2 text-sm"><dt className="text-slate-500">{c.localHost}</dt><dd>{nvr.local_host}</dd><dt className="text-slate-500">{t.agents.title}</dt><dd>{gatewayName(nvr.gateway_id)}</dd><dt className="text-slate-500">{c.cameras}</dt><dd>{cameras.filter((item) => item.nvr_connection_id === nvr.id).length}</dd><dt className="text-slate-500">{c.lastTested}</dt><dd>{nvr.last_tested_at ? new Intl.DateTimeFormat(language).format(new Date(nvr.last_tested_at)) : c.never}</dd></dl><p className="mt-4 text-xs text-amber-200">{c.agentNotice}</p>{canManageNvrs && <div className="mt-4 flex gap-2"><button onClick={() => setForm({ id:nvr.id,locationId:nvr.location_id,gatewayId:nvr.gateway_id??'',name:nvr.name,vendor:nvr.vendor,localHost:nvr.local_host,httpPort:String(nvr.http_port??''),rtspPort:String(nvr.rtsp_port??''),onvifPort:String(nvr.onvif_port??''),usernameSecretReference:'',passwordSecretReference:'',status:nvr.status === 'configured' ? 'configured' : 'unconfigured' })} className="min-h-11 rounded-xl border border-white/10 px-4">{c.edit}</button><button onClick={() => void removeNvr(nvr.id)} disabled={saving} className="min-h-11 rounded-xl border border-rose-400/30 px-4 text-rose-200">{c.remove}</button></div>}</article>)}</div>}</div>
      <div><h2 className="mb-3 text-xl font-bold">{c.cameras}</h2>{cameras.length === 0 ? <div className="rounded-3xl border border-dashed border-white/15 p-8 text-center text-slate-400"><Camera className="mx-auto mb-3 h-8 w-8" />{c.noCameras}</div> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{cameras.map((item) => <article key={item.id} className="rounded-2xl border border-white/10 bg-slate-950/70 p-5"><div className="flex justify-between gap-3"><h3 className="font-bold">{item.name}</h3><span className="text-xs text-slate-400">{c.channel} {item.external_channel_id}</span></div><p className="mt-2 text-sm text-slate-400">{locationName(item.location_id)} · {nvrName(item.nvr_connection_id)}</p><p className="mt-2 text-sm">{item.area || c.unassigned} / {item.department || c.unassigned}</p><div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-slate-800 px-3 py-1">{c.status[item.status as keyof typeof c.status] ?? item.status}</span>{item.ai_enabled && <span className="rounded-full bg-cyan-500/15 px-3 py-1 text-cyan-200">{c.aiEnabled}</span>}{item.task_verification_enabled && <span className="rounded-full bg-violet-500/15 px-3 py-1 text-violet-200">{c.taskVerification}</span>}</div><button onClick={() => setEditingCamera({ ...item })} className="mt-4 min-h-11 rounded-xl border border-white/10 px-4">{c.edit}</button></article>)}</div>}</div>
    </>}
    {form && <div role="dialog" aria-modal="true" aria-labelledby="nvr-title" className="fixed inset-0 z-50 flex items-end justify-center bg-black/75 p-3 sm:items-center">
      <form onSubmit={saveNvr} className="max-h-[calc(100dvh-1.5rem)] w-full max-w-xl overflow-y-auto rounded-3xl border border-white/10 bg-slate-950 p-5">
        ...
        <div className="mt-5 flex justify-end gap-2">
          <button type="button" onClick={() => setForm(null)} className="min-h-11 px-4">{c.cancel}</button>
          <button disabled={saving || locations.length === 0} className="min-h-11 rounded-xl bg-cyan-400 px-5 font-semibold text-slate-950">{saving ? c.saving : c.save}</button>
        </div>
      </form>
    </div>}
    {editingCamera && <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-end justify-center bg-black/75 p-3 sm:items-center"><form onSubmit={saveCamera} className="w-full max-w-lg rounded-3xl border border-white/10 bg-slate-950 p-5"><h2 className="text-xl font-bold">{c.editCamera}</h2>{(['name','area','department'] as const).map((key)=><label key={key} className="mt-3 block">{c[key]}<input required={key==='name'} value={editingCamera[key]??''} onChange={(e)=>setEditingCamera({...editingCamera,[key]:e.target.value||null})} className="mt-1 min-h-11 w-full rounded-xl bg-slate-900 px-3" /></label>)}<label className="mt-4 flex min-h-11 items-center gap-3"><input type="checkbox" checked={editingCamera.ai_enabled} onChange={(e)=>setEditingCamera({...editingCamera,ai_enabled:e.target.checked})}/>{c.aiEnabled}</label><label className="flex min-h-11 items-center gap-3"><input type="checkbox" checked={editingCamera.task_verification_enabled} onChange={(e)=>setEditingCamera({...editingCamera,task_verification_enabled:e.target.checked})}/>{c.taskVerification}</label><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={()=>setEditingCamera(null)} className="min-h-11 px-4">{c.cancel}</button><button disabled={saving} className="min-h-11 rounded-xl bg-cyan-400 px-5 font-semibold text-slate-950">{saving?c.saving:c.save}</button></div></form></div>}
  </section>;
}
