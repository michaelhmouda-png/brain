...
export default function CamerasPage() {
  ...
  return <section dir={language === 'ar' ? 'rtl' : 'ltr'} className="space-y-6 px-4 pb-8 sm:px-6 lg:px-0">
    <header className="flex flex-col gap-4 rounded-[28px] border border-black bg-white p-5 sm:flex-row sm:items-end sm:justify-between sm:p-7">
      <div><p className="text-sm font-semibold uppercase tracking-[0.25em] text-black">{c.eyebrow}</p><h1 className="mt-2 text-3xl font-black text-black sm:text-4xl">{c.title}</h1><p className="mt-2 max-w-2xl text-sm text-zinc-800">{c.description}</p></div>
    ...
    {loading ? <div className="rounded-3xl border border-white/10 p-8 text-center text-slate-300">{c.loading}</div> : <>
      <div>
        ...
        {visibleNvrs.length === 0 ? <div className="rounded-3xl border border-dashed border-black bg-white p-8 text-center text-zinc-800"><Server className="mx-auto mb-3 h-8 w-8" />{locations.length === 0 ? c.noLocations : c.noNvrs}</div> : <div className="grid gap-3 lg:grid-cols-2">{visibleNvrs.map((nvr) => <article key={nvr.id} className="rounded-2xl border border-black bg-white p-5 text-black"><div className="flex justify-between gap-3"><div><h3 className="font-bold">{nvr.name}</h3><p className="text-sm text-zinc-800">{nvr.vendor} · {locationName(nvr.location_id)}</p></div><StatusBadge tone={deviceStatusTone(nvr.status)} label={c.status[nvr.status as keyof typeof c.status] ?? nvr.status} /></div><dl className="mt-4 grid grid-cols-2 gap-2 text-sm"><dt className="text-zinc-700">{c.localHost}</dt><dd>{nvr.local_host}</dd><dt className="text-zinc-700">{t.agents.title}</dt><dd>{gatewayName(nvr.gateway_id)}</dd><dt className="text-zinc-700">{c.cameras}</dt><dd>{cameras.filter((item) => item.nvr_connection_id === nvr.id).length}</dd><dt className="text-zinc-700">{c.lastTested}</dt><dd>{nvr.last_tested_at ? new Intl.DateTimeFormat(language).format(new Date(nvr.last_tested_at)) : c.never}</dd></dl><p className="ui-alert ui-alert-warning mt-4 rounded-xl p-3 text-xs">{c.agentNotice}</p>{canManageNvrs && <NvrProbeControls nvrConnectionId={nvr.id} />}{canManageNvrs && <div className="mt-4 flex gap-2"><button onClick={() => setForm({ id:nvr.id,locationId:nvr.location_id,gatewayId:nvr.gateway_id??'',name:nvr.name,vendor:nvr.vendor,localHost:nvr.local_host,httpPort:String(nvr.http_port??''),rtspPort:String(nvr.rtsp_port??''),onvifPort:String(nvr.onvif_port??''),usernameSecretReference:'',passwordSecretReference:'',status:nvr.status === 'configured' ? 'configured' : 'unconfigured' })} className="ui-button-primary min-h-11 rounded-xl px-4">{c.edit}</button><button onClick={() => void removeNvr(nvr.id)} disabled={saving} className="ui-button-destructive min-h-11 rounded-xl px-4">{c.remove}</button></div>}</article>)}</div>}
      </div>
      <div><h2 className="mb-3 text-xl font-bold">{c.cameras}</h2>{cameras.length === 0 ? <div className="rounded-3xl border border-dashed border-black bg-white p-8 text-center text-zinc-800"><Camera className="mx-auto mb-3 h-8 w-8" />{c.noCameras}</div> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{cameras.map((item) => <article key={item.id} className="rounded-2xl border border-black bg-white p-5 text-black"><div className="flex justify-between gap-3"><h3 className="font-bold">{item.name}</h3><span className="text-xs text-zinc-700">{c.channel} {item.external_channel_id}</span></div><p className="mt-2 text-sm text-zinc-800">{locationName(item.location_id)} · {nvrName(item.nvr_connection_id)}</p><p className="mt-2 text-sm">{item.area || c.unassigned} / {item.department || c.unassigned}</p><div className="mt-3 flex flex-wrap gap-2 text-xs"><StatusBadge tone={deviceStatusTone(item.status)} label={c.status[item.status as keyof typeof c.status] ?? item.status} />{item.ai_enabled && <StatusBadge tone="info" label={c.aiEnabled} />}{item.task_verification_enabled && <StatusBadge tone="review" label={c.taskVerification} />}</div><button onClick={() => setEditingCamera({ ...item })} className="ui-button-primary mt-4 min-h-11 rounded-xl px-4">{c.edit}</button>{canManageNvrs && nvrs.find((nvr) => nvr.id === item.nvr_connection_id)?.gateway_id ? <CameraSnapshotControl gatewayId={nvrs.find((nvr) => nvr.id === item.nvr_connection_id)!.gateway_id!} nvrConnectionId={item.nvr_connection_id} channelId={item.external_channel_id} /> : null}</article>)}</div>}</div>
    </>}
    {form && <div role="dialog" aria-modal="true" aria-labelledby="nvr-title" className="ui-overlay fixed inset-0 z-50 flex items-end justify-center p-3 sm:items-center">
      <form onSubmit={saveNvr} className="ui-inverse max-h-[calc(100dvh-1.5rem)] w-full max-w-xl overflow-y-auto rounded-3xl p-5">
        ...
        <p className="mt-3 text-xs text-slate-200">{c.secretNotice}</p>
