...
import { getMessages, interpolateMessage, normalizeLanguage } from '@/lib/i18n';
...
export default async function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const { data: company } = profile.company_id
    ? await supabase.from('companies').select('name, timezone').eq('id', profile.company_id).maybeSingle()
    : { data: null };

  const language = normalizeLanguage(profile.preferred_language);
  const t = getMessages(language);
  if (profile.status !== 'active') {
    const status = profile.status in t.profileStatus
      ? t.profileStatus[profile.status as keyof typeof t.profileStatus]
      : profile.status;
    return (
      <div lang={language} dir={language === 'ar' ? 'rtl' : 'ltr'} className="min-h-screen overflow-hidden bg-[#020202] text-white">
        <div className="relative mx-auto flex min-h-screen max-w-[1700px] items-center justify-center px-4 py-6">
          <div className="max-w-md rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur-xl">
            <h1 className="text-2xl font-bold text-white">{t.accountState.inactive}</h1>
            <p className="mt-4 text-slate-400">{interpolateMessage(t.accountState.inactiveHelp, { status })}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <LocaleProvider language={language} role={profile.role} companyTimezone={company?.timezone ?? 'UTC'}>
