...
export default function EvidenceReviewPage() {
  ...
  return <main className="space-y-5 text-white"><div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-bold">Evidence review</h1><p className="text-sm text-slate-400">AI results support a human decision. Manager approval completes the linked active task; AI verification alone never changes task status.</p></div><button onClick={() => void load()} className="min-h-11 rounded-xl border border-white/10 px-4">Refresh</button></div>
    ...
    {confirming && <div className="ui-overlay fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:p-4" role="dialog" aria-modal="true"><div className="ui-management-surface w-full rounded-t-3xl border p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] sm:max-w-md sm:rounded-2xl"><h2 className="text-lg font-bold">Confirm {confirming.decision}</h2><p className="ui-muted mt-2 text-sm">{confirming.decision === 'approved' ? 'Approve evidence and complete this task?' : 'Reject evidence and leave this task unchanged?'}</p><label className="mt-4 block text-sm font-semibold">Optional note<textarea value={note} maxLength={1000} onChange={(event) => setNote(event.target.value)} className="ui-field mt-1 min-h-24 w-full rounded-xl p-3 text-base" /></label><div className="mt-4 grid grid-cols-2 gap-2"><button disabled={saving} onClick={() => setConfirming(null)} className="ui-button-secondary">Cancel</button><button disabled={saving} onClick={() => void review()} className="ui-button-primary">{saving ? 'Saving…' : 'Confirm'}</button></div></div></div>}
  </main>;
}
