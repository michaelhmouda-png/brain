...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const requestedItemId = new URL(request.url).searchParams.get('itemId');
  if (requestedItemId !== null && !isUuid(requestedItemId)) {
    return NextResponse.json({ error: 'Invalid evidence item identifier' }, { status: 400, headers: NO_STORE_HEADERS });
  }
  const { data, error } = requestedItemId
    ? await supabase.rpc('get_task_evidence_item_access', {
        p_evidence_id: id,
        p_item_id: requestedItemId,
      })
    : await supabase.rpc('get_task_evidence_access', { p_evidence_id: id });
