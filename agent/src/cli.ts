    ...
    return await new Promise<string>((resolve, reject) => {
      const onData = (chunk: string | Buffer) => {
        ...
        for (const character of text) {
          ...
          else if (/^[0-9a-fA-F]$/.test(character) && value.length < 32) value += character;
        }
      };
...
async function main() {
  ...
  throw new Error('Usage: pair | start | status | unpair-local');
}
