...
captured_cron_jobs AS (
  SELECT
    ...
    job.database,
    job.username,
    octet_length(convert_to(job.command, 'UTF8')) AS command_utf8_bytes,
    pg_catalog.encode(
      extensions.digest(convert_to(job.command, 'UTF8'), 'sha256'),
      'hex'
    ) AS production_command_sha256,
    regexp_replace(
      regexp_replace(
        regexp_replace(
          regexp_replace(
    ...
    CASE job.jobname
      WHEN 'camera-evidence-worker-every-minute'
        THEN 'supabase/migrations/202607210008_camera_evidence_worker_schedule.sql'
      WHEN 'notification-worker-every-minute'
        THEN 'supabase/migrations/202607210009_notification_foundation_n1.sql'
      ELSE null
    END AS repository_source_file
