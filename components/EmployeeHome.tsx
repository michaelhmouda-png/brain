...
export function EmployeeHome() {
  ...
  useEffect(() => { const controller = new AbortController(); Promise.all([
    ...
    setSummary({ tasks: tasks.length, today: tasks.filter((task) => task.dueDate === today && task.status !== 'completed').length, overdue: tasks.filter((task) => typeof task.dueDate === 'string' && task.dueDate < today && task.status !== 'completed').length, notifications: notificationPayload && typeof notificationPayload === 'object' && 'unread_count' in notificationPayload ? Number((notificationPayload as { unread_count: unknown }).unread_count) || 0 : 0 });
