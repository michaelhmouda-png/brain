...
export function EmployeeHome() {
  ...
  return <section className="space-y-6 rounded-[32px] border border-white/10 bg-white/5 p-5 sm:p-8">
    ...
    ].map(([label, value]) => <article key={String(label)} className="rounded-2xl border border-white/10 bg-slate-950/60 p-4"><p className="text-sm text-slate-400">{label}</p><p className="mt-2 text-3xl font-black text-white">{typeof value === 'number' ? new Intl.NumberFormat(language === 'ar' ? 'ar-LB' : 'en').format(value) : '—'}</p></article>)}</div>
    <div className="grid gap-3 sm:grid-cols-3"><Link href="/dashboard/tasks" className="flex min-h-11 items-center justify-center rounded-xl bg-cyan-600 px-4 font-semibold">{t.home.viewTasks}</Link><Link href="/dashboard/notifications" className="flex min-h-11 items-center justify-center rounded-xl border border-white/15 px-4 font-semibold">{t.home.notifications}</Link><button type="button" onClick={() => window.dispatchEvent(new Event('brain:open'))} className="flex min-h-11 items-center justify-center rounded-xl border border-white/15 px-4 font-semibold">{t.home.askBrain}</button></div>
  </section>;
}
