...
function booleanPreference(row: Record<string, unknown>, key: string, fallback: boolean) {
...
export function NotificationSettings() {
  ...
  useEffect(() => {
    Promise.resolve().then(() => {
      ...
      setState(Notification.permission === 'denied' ? 'denied' : Notification.permission === 'granted' ? 'expired' : 'not_requested');
  ...
  async function enable() {
    ...
    try {
      ...
      if (permission !== 'granted') { setState(permission === 'denied' ? 'denied' : 'not_requested'); return; }
      const registration = await navigator.serviceWorker.register('/notification-sw.js', { scope: '/' });
      const keyResponse = await fetch('/api/notifications/vapid-key', { cache: 'no-store', credentials: 'same-origin' });
      const key: unknown = await keyResponse.json();
      if (!keyResponse.ok || typeof key !== 'object' || key === null || !('publicKey' in key) || typeof key.publicKey !== 'string') throw new Error('VAPID_KEY_UNAVAILABLE');
      const subscription = await registration.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey: bytes(key.publicKey) });
      const json = subscription.toJSON();
      if (!json.endpoint || !json.keys?.p256dh || !json.keys.auth) throw new Error('INVALID_PUSH_SUBSCRIPTION');
      const response = await fetch('/api/notifications', {
        method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'subscribe', endpoint: json.endpoint, p256dh: json.keys.p256dh, auth: json.keys.auth, device: navigator.userAgent.slice(0, 80) }),
      });
  ...
  async function disable() {
    ...
    if (subscription) {
      await fetch('/api/notifications', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'revoke', endpoint: subscription.endpoint }) });
      await subscription.unsubscribe();
    }
    setState('not_requested'); setPrefs((current) => ({ ...current, pushEnabled: false }));
  }
